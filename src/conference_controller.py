...
def join_conference():
    ...
    current_app.logger.info(
        "🎪 join_conference invoked", extra={"params": request.args.to_dict()}
    )
    caller_identity = request.args.get("identity", None)
    start_conference_on_enter = str2bool(
        request.args.get("start_conference_on_enter"), True
    )
    end_conference_on_exit = str2bool(
        request.args.get("end_conference_on_exit"), True
    )
    muted = str2bool(request.args.get("mute"), False)
    participant_label = request.args.get(
        "participant_label", "DefaultParticipant"
    )
    stream_audio = str2bool(request.args.get("stream_audio"), False)
    hold_on_conference_join = str2bool(
        request.args.get("hold_on_conference_join"), False
    )
    play_temporary_greeting = str2bool(
        request.args.get("play_temporary_greeting"), False
    )
    ...
    def sc_url():
        ...
        if caller_identity:
            current_app.logger.debug(
                "🎪 Caller identity present for conference: %s", conference_name
            )
    ...
    dial.conference(
        ...
        recording_status_callback=url_for(
            "conference.conference_recording_events", _external=True
        ),
...
def connect_to_conference():
    conference_name = request.args.get("conference_name", "DefaultRoom")
    current_app.logger.info(
        "🎪 connect_to_conference invoked",
        extra={"params": request.args.to_dict()},
    )
    response = VoiceResponse()
...
def conference_announcement():
    response = VoiceResponse()
    response.say(
        "You are being joined back into the call.",
        voice="alice",
        language="en-US",
    )
    return xml_response(response)
...
@conference_bp.route(
    "/conference/<conference_name>/participants", methods=["GET"]
)
def get_conference_participants(conference_name):
    current_app.logger.info(
        "🎪 get_conference_participants invoked",
        extra={"conference_name": conference_name},
    )
    ...
    current_app.logger.info(
        "🎪 get_conference_participants processing complete"
    )
...
def mute_participant():
    ...
    current_app.logger.info(
        "🎪 mute_participant invoked", extra={"payload": data}
    )
    ...
    try:
        client.conferences(conf_sid).participants(call_sid).update(
            muted=bool(mute)
        )
...
def hold_participant():
    data = request.get_json()
    current_app.logger.info(
        "🎪 hold_participant invoked", extra={"payload": data}
    )
    conference_name = data.get("conference_name")
    call_sid = data.get("call_sid")
...
def kick_participant():
    data = request.get_json()
    current_app.logger.info(
        "🎪 kick_participant invoked", extra={"payload": data}
    )
    conference_name = data.get("conference_name")
    call_sid = data.get("call_sid")

    requester_call_sid = request.args.get("call_sid") or data.get(
        "requester_call_sid"
    )
    if requester_call_sid and call_sid == requester_call_sid:
        return (
            jsonify({"success": False, "error": "You cannot kick yourself."}),
            400,
        )
    redis = current_app.config["redis"]
    client = current_app.config["twilio_client"]
...
def conference_recording_events():
    ...
    current_app.logger.info(
        "🎪 conference_recording_events endpoint processing complete"
    )
